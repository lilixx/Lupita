...
<body>
  ...
  <div class="col1_of_2">
    ...
    <div class="col2_of_2">
    @if($prestamo->socio->empresa_id != null)
...
<h1 style="text-align:center;">Detalle del Financiamiento</h1>
  ...
  <div class="col1_of_2">
  <p>  Cuotas Mensuales: {{$prestamo->cantcuotas}}<br/>
  Cuota Mensual (Dólares): {{$prestamo->cuota}}<br/>
...
<h1 style="text-align:center;">Detalle de Cuotas</h1>
  ...
  @while ($saldo >= 0 && $i <= $prestamo->cantcuotas)
    <tr>
      ...
      <td class="tg-yw4l">{{number_format($interes = $saldo * 0.02, 2)}}</td>
      ...
      <td class="tg-yw4l">{{number_format($saldo = $saldo - $principal, 2)}}</td> <span style="display:none;"> @if($i > 1){{$mes = $mes+1}} @endif</span>
      <td class="tg-baqh">  @if($mes == 2 && $dia == 30) {{$diaf = 28}} @else {{$dia}} @endif </td>
